...
async function main() {
  ...
  // Inject all writers into NotificationService
  services.addScoped<INotificationService>(
    INotificationServiceToken,
    NotificationService,
    [IMessageWriterToken], // All IMessageWriter implementations will be injected
  );

  // But this won't work because dependencies array expects a single service
  // Instead, we should use factory or getServices
